    ...
    <div class="p-3 space-y-2">
      <div>
        ...
        <div class="flex flex-wrap gap-1">
          ...
          <span v-if="queryTags.length === 0" class="text-xs text-muted-foreground">No conditions</span>
        </div>
      </div>

      <div>
        <p class="text-[10px] uppercase text-muted-foreground mb-1">Actions</p>
        <div class="flex flex-wrap gap-1">
          <Badge
            v-for="(action, i) in actionSummary"
            :key="i"
            variant="secondary"
            class="text-[10px]"
          >
          ...
          <span v-if="actionSummary.length === 0" class="text-xs text-muted-foreground">No actions</span>
        </div>
      </div>
